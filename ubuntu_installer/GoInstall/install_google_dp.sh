go get gitlab.com/jinfagang/colorgo

go get github.com/gomodule/redigo/redis
go get github.com/BurntSushi/toml

go get github.com/dgrijalva/jwt-go
go get github.com/gin-gonic/gin
go get github.com/jinzhu/gorm
go get github.com/sirupsen/logrus
go get github.com/lib/pq
go get github.com/nfnt/resize

git clone https://github.com/dgrijalva/jwt-go $HOME/go/src/github.com/dgrijalva/jwt-go
cd $HOME/go/src/github.com/dgrijalva/jwt-go
git checkout release_4_0_0

git clone https://github.com/golang/net.git $HOME/go/src/golang.org/x/net
git clone https://github.com/golang/sys.git $HOME/go/src/golang.org/x/sys
git clone https://github.com/golang/crypto.git $HOME/go/src/golang.org/x/crypto
git clone https://github.com/golang/tools.git $HOME/go/src/golang.org/x/tools
git clone https://github.com/grpc/grpc-go.git $HOME/go/src/google.golang.org/grpc
git clone https://github.com/golang/text.git $HOME/go/src/golang.org/x/text
git clone https://github.com/google/go-genproto.git $HOME/go/src/google.golang.org/genproto
